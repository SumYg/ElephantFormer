...
- **MCTS bot** (`--mcts_sims N`, combined ckpt): **10-0-0 vs greedy at 200 sims, all checkmates** — no stalemate-trap loss, no move-cap draws; both 1-ply blind spots close in real play. ~3-4 s/move on CPU at 200 sims; cross-tree leaf batching is the future speedup.
...
- **Ladder baseline** (combined ckpt + rerank, paired openings, repetition-aware engine): vs **Pikafish@256** = **0W-40L-0D, Elo ≤ −712** (95% bound; all checkmates, 0 rules disagreements in 40 games). The model is below the 256-node rung; lower rungs (1–128 nodes) queued to find the measurable part of the curve. An earlier 1W-9L probe was flattered by engine repetition-blindness (bare FEN, since fixed).
...
- [x] MCTS at inference: **Gumbel sequential halving** at the root (deterministic at `gumbel_scale 0` for eval; noise ready for self-play), PUCT below, exact in-tree terminals (mate/困毙 = loss, 3-fold repetition = draw), root-round batched leaf evaluation — `board_match`/`ladder` `--mcts_sims N [--mcts_top_m M]`. Verified: finds mate-in-1, avoids mate-in-2 traps that 1-ply provably cannot (tests/test_board_mcts.py). Full cross-tree leaf batching left as a throughput optimisation
- [x] Eval harness v2 (spec below): **ladder runner done** — `python -m elephant_former.evaluation.ladder --model_path <ckpt> [--rerank] --nodes 256 1024 ... --num_games N [--sprt --elo0 0 --elo1 20]`; paired games from `data/openings_wxf_top100.json` (top 6-ply prefixes, 69% corpus coverage, rebuild with `scripts/build_opening_book.py`), trinomial GSPRT, Elo ± 95% CI, JSONL records to `logs/ladder.jsonl`; engine now sees full move history (repetition-aware). Elo-vs-N *chart* still to make once ≥2 model generations exist
